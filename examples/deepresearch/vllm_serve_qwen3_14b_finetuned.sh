...
# srun --nodes=1 --ntasks=1 --overlap -w "$head_node" vllm serve ${SANDMLE_CKPT_DIR:-checkpoints}/sandmle/sandmle_qwen3_14b_mle_syn_single_node_filter_timeout/global_step_85/actor/huggingface \
#   --trust-remote-code \
#   --seed=1 \
#   --port 8001 \
...
# srun --nodes=1 --ntasks=1 --overlap -w "$head_node" vllm serve ${SANDMLE_CKPT_DIR:-checkpoints}/sandmle/sandmle_qwen3_14b_mle_sft_single_node/global_step_40/huggingface \
#   --trust-remote-code \
#   --seed=1 \
#   --port 8001 \
#   --served-model-name "qwen3_14b_serve_finetuned" \
...
srun --nodes=1 --ntasks=1 --overlap -w "$head_node" vllm serve ${SANDMLE_CKPT_DIR:-checkpoints}/sandmle/sandmle_qwen3_14b_mle_syn_single_node_no_milestone/global_step_80/actor/huggingface \
  --trust-remote-code \
  --seed=1 \
  --port 8001 \
  --served-model-name "qwen3_14b_serve_no_mile_stone_finetuned" \
  --tensor-parallel-size=$SLURM_GPUS_PER_TASK \
  --gpu-memory-utilization 0.95 \
  --enforce-eager