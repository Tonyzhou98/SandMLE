...
class DeepResearchWorkflow(Workflow):
    ...
    def _score_to_reward(self, metrics: dict, message_count: int, format_correct_rate: float) -> tuple[float, dict]:
        ...
        # Reward mode: "dense" (default, milestone-based) or "sparse" (format + gold only).
        # See the reward-design ablation in the paper appendix. Controlled by SANDMLE_REWARD.
        reward_type = os.environ.get("SANDMLE_REWARD", "dense").lower()
        if reward_type == "sparse":
            return self._sparse_reward(score, lower_is_better, metrics, message_count, format_correct_rate)
    ...
    def _sparse_reward(self, score, lower_is_better, metrics, message_count, format_correct_rate):
        """Sparse reward baseline from the reward-design ablation: r = 0.1*format + 0.9*I_gold.

        Only format compliance and reaching the highest (gold) performance tier are rewarded;
        there is no dense credit for execution or intermediate median/bronze/silver milestones.
        """
        reward = 0.1 * format_correct_rate
