...
#SBATCH --output=slurm/mle_syn_qwen3_235b_megatron_rl_grpo_agent_multi_nodes.stdout
#SBATCH --error=slurm/mle_syn_qwen3_235b_megatron_rl_grpo_agent_multi_nodes.stderr
...
# Avoid flashinfer/vLLM JIT lock files on shared filesystem.
# Keep all JIT/cache artifacts on node-local disk.
export LOCAL_CACHE_ROOT=/tmp/${USER}/rllm-${SLURM_JOB_ID}
export FLASHINFER_WORKSPACE_BASE=${LOCAL_CACHE_ROOT}/flashinfer
...
export SRUN_API_URL="${SRUN_API_URL:-http://127.0.0.1:9000}"

mkdir -p "${FLASHINFER_WORKSPACE_BASE}" "${TRITON_CACHE_DIR}" "${TORCH_EXTENSIONS_DIR}" "${XDG_CACHE_HOME}"

CHECKPOINT_PATH=${SANDMLE_CKPT_DIR:-checkpoints}
DATA_PATH=${SANDMLE_DATASETS_DIR:-data/datasets}
project_name="sandmle"
experiment_name="sandmle_qwen3_235b_mle_syn_multi_nodes_megatron"

# 8-node (64 GPU) high-PP profile for lower per-rank model memory:
# TP * PP * EP = 4 * 8 * 2 = 64 (DP=1 on 64 GPUs)
gen_tp=8
train_tp=4
train_pp=8
train_ep=2
...
# (94 - last_layer) % (8 - 1) == 0 => choose last_layer=4.
# This yields 84 middle layers across 7 stages (12 each), a balanced split.
last_layer=10
...
PYTHONUNBUFFERED=1 srun --overlap --nodes=1 --ntasks=1 -w "$head_node" \
  bash -c "
    run_root=${SANDMLE_OUTPUT_ROOT:-examples/deepresearch/output}
    ts=\$(date +%Y%m%d-%H%M%S)
    ...
    python3 -m examples.deepresearch.custom_train_megatron \
      ...
      actor_rollout_ref.model.path=${SANDMLE_MODEL_DIR:-Qwen}/Qwen3-235B-A22B-Thinking-2507 \
