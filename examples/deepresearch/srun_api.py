...
DEFAULT_CONDA_ENV = os.environ.get("SANDMLE_CONDA_ENV", "sandmle")
...
class RunRequest(BaseModel):
    ...
    job_name: Optional[str] = "sandmle_exec_job"
...
async def run(req: RunRequest = Body(..., example={"script_path": "/path/to/main.py"})):
    ...
    conda_activate = f"source {shlex.quote(str(Path(CONDA_BASE) / 'bin' / 'activate'))} && conda activate {shlex.quote(req.conda_env or DEFAULT_CONDA_ENV)}"
    ...
    srun_cmd = [
        ...
        "--mem=100G",
        "--time=2-00:00:00",
        f"--job-name={req.job_name or 'sandmle_exec_job'}",
        "bash",
        "-lc",
        inner_cmd,
    ]
