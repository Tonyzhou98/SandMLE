...
- Overall runtime limits: the agent may take up to {max_llm_calls} turns.
...
def build_tasks(competition_ids: list[str], data_root: Path, args: argparse.Namespace) -> list[dict]:
    ...
    for competition_id in competition_ids:
        ...
        tasks.append(
            {
                "question": user_prompt_template.replace(
                    "{specific_task_description}", specific_prompt
                ).replace("{max_llm_calls}", str(args.max_llm_calls))
            }
        )
    return tasks
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            "Dataset selection: "
            "'easy' uses the real MLE-bench data with the fixed competition id list; "
            "'hard' uses all subfolders under SANDMLE_DATA_ROOT excluding those competition ids "
            "(prints the excluded ids); "
            "'synthetic' uses SANDMLE_SYN_DATA_ROOT with SynScoreTool."
        ),
    )
    parser.add_argument(
        "--max-llm-calls",
...
def create_rollout_engine(model: str) -> OpenAIEngine:
    """Create the rollout engine for the desired policy model.

    - Hosted/proprietary reference models (Claude/GPT/Gemini/DeepSeek) are routed through
      OpenRouter using ``OPENROUTER_API_KEY``.
    - Any other value is treated as a locally served (base or finetuned) policy: the engine
      talks to the vLLM OpenAI-compatible server configured via ``SANDMLE_MODEL_BASE_URL``
      and ``SANDMLE_SERVED_MODEL_NAME`` (see vllm_serve_*.sh and .env.example).
    """
    ...
    return OpenAIEngine(
        ...
        api_key=os.environ.get("SANDMLE_MODEL_API_KEY", "None"),
...
async def main():
    ...
    data_root = cfg.SYN_DATA_ROOT if "synthetic" in args.dataset else cfg.DATA_ROOT
    ...
    if "synthetic" in args.dataset:
        ...
        if args.dataset == "synthetic":
            ok_list_path = cfg.OK_LIST
        elif args.dataset == "synthetic_sampled":
            ok_list_path = cfg.OK_LIST_SAMPLED
